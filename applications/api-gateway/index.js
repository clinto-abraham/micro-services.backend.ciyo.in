...
// const DATABASE_URL = parseInt(process.env.DATABASE_URL || "5454", 10);
...
app.get("/health", async (req, res) => {
  ...
  const serviceList = {
    ...
    data: process.env.API_SVC || "http://localhost:4000",
    ai: process.env.AI_SVC || "http://localhost:9000",
...
try {
  // const redisCheck = await redis.redis.ping(); 
  // results.databases.redis = { ok: redisCheck === "PONG" };
  const res = await redis.checkRedisStatus()
  console.log(res, "redis ", 73)
     results.databases.redis = await redis.checkRedisStatus()
} catch (e) {
